...
import { AddIcon, SortIcon, SortAscIcon, SortDescIcon, SearchIcon, ChevronLeftIcon, ChevronRightIcon, UserGroupIcon, EditIconAlt, DeleteIconAlt } from '../../components/icons';
import { User } from '../../types';
import UserFormModal from './UserFormModal';

const MySwal = withReactContent(Swal);

type SortableKeys = keyof Omit<User, 'id' | 'password'>;
...
const UserListPage: React.FC = () => {
    ...
    const startEntry = processedUsers.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-xl border border-slate-200">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-slate-600">
                        <thead className="text-xs font-semibold text-sidebar-text bg-sidebar-bg uppercase">
                            <tr>
                                <SortableHeader column="username" title="Nom d'utilisateur" sortConfig={sortConfig} requestSort={requestSort} />
                                <SortableHeader column="fullName" title="Nom et Prénoms" sortConfig={sortConfig} requestSort={requestSort} />
                        ...
                        <tbody className="align-middle">
                            {paginatedUsers.length > 0 ? paginatedUsers.map((user, index) => (
                                <tr key={user.id} className={`border-t border-slate-200 hover:bg-primary/5 transition-colors duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/70'}`}>
                                    <th scope="row" className="px-6 py-3 font-medium text-slate-900 whitespace-nowrap">{user.username}</th>
                                    <td className="px-6 py-3">{user.fullName}</td>
                                    <td className="px-6 py-3">{user.email}</td>
                                    <td className="px-6 py-3">{user.contact}</td>
                                    <td className="px-6 py-3">
                                        <div className="flex items-center justify-center space-x-2">
                                            <button onClick={() => handleOpenModal(user.id)} className="p-1.5 text-blue-600 bg-blue-100 hover:bg-blue-200 rounded-md transition-colors" aria-label={`Modifier l'utilisateur ${user.username}`}>
                                                <EditIconAlt className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => handleDelete(user.id, user.username)} className="p-1.5 text-red-600 bg-red-100 hover:bg-red-200 rounded-md transition-colors" aria-label={`Supprimer l'utilisateur ${user.username}`}>
                                ...
                                <tr>
                                    <td colSpan={5} className="text-center py-16 text-slate-500">
                                        ...
                                        <p className="font-semibold mt-4 text-lg">Aucun utilisateur trouvé</p>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center p-4 border-t border-slate-200 rounded-b-xl">
                    <p className="text-sm text-slate-600 mb-4 sm:mb-0">
                        Affichage de <span className="font-semibold">{startEntry}</span> à <span className="font-semibold">{endEntry}</span> sur <span className="font-semibold">{processedUsers.length}</span> éléments
                    </p>
                    {totalPages > 1 && (
                         <nav>
            ...
            <UserFormModal isOpen={isModalOpen} onClose={handleCloseModal} userId={editingUserId} />
        </div>
    );
};
