...
import { AddIcon, SortIcon, SortAscIcon, SortDescIcon, SearchIcon, ChevronLeftIcon, ChevronRightIcon, CubeIcon, EditIconAlt, DeleteIconAlt } from '../../components/icons';
import { Produit } from '../../types';
import ProduitFormModal from './ProduitFormModal';

const MySwal = withReactContent(Swal);

type SortableKeys = keyof Omit<Produit, 'id'>;
...
const ProduitListPage: React.FC = () => {
    const { produits, deleteProduit, fetchProduits } = useProduits();
    ...
    const processedItems = useMemo(() => {
        let filteredItems = [...produits];

        if (searchTerm) {
            const lowercasedFilter = searchTerm.toLowerCase();
            filteredItems = filteredItems.filter(item =>
                item.nomProduit.toLowerCase().includes(lowercasedFilter)
            );
        }
    ...
    const handleDelete = (id: string, name: string) => {
        MySwal.fire({
            title: 'Êtes-vous sûr ?',
            text: `Vous êtes sur le point de supprimer le produit ${name}. Cette action est irréversible !`,
            icon: 'warning',
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-slate-600">
                        <thead className="text-xs font-semibold text-sidebar-text bg-sidebar-bg uppercase">
                            <tr>
                                <SortableHeader column="nomProduit" title="Produit" sortConfig={sortConfig} requestSort={requestSort} />
                        ...
                        <tbody className="align-middle">
                            {paginatedItems.length > 0 ? paginatedItems.map((item, index) => (
                                <tr key={item.id} className={`border-t border-slate-200 hover:bg-primary/5 transition-colors duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/70'}`}>
                                    <th scope="row" className="px-6 py-3 font-medium text-slate-900 whitespace-nowrap">{item.nomProduit}</th>
                                    <td className="px-6 py-3">
                                        <div className="flex items-center justify-center space-x-2">
                                            <button onClick={() => handleOpenModal(item.id)} className="p-1.5 text-blue-600 bg-blue-100 hover:bg-blue-200 rounded-md transition-colors" aria-label={`Modifier le produit ${item.nomProduit}`}>
                                                <EditIconAlt className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => handleDelete(item.id, item.nomProduit)} className="p-1.5 text-red-600 bg-red-100 hover:bg-red-200 rounded-md transition-colors" aria-label={`Supprimer le produit ${item.nomProduit}`}>
                                ...
                                <tr>
                                    <td colSpan={2} className="text-center py-16 text-slate-500">
                                        ...
                                        <p className="font-semibold mt-4 text-lg">Aucun produit trouvé</p>
                                        <p className="text-sm mt-1">{searchTerm ? 'Essayez de modifier vos critères de recherche.' : 'Créez un nouveau produit pour commencer.'}</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
            ...
            <ProduitFormModal isOpen={isModalOpen} onClose={handleCloseModal} produitId={editingId} />
        </div>
    );
};
