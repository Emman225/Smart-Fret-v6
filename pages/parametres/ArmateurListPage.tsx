...
type SortableKeys = keyof Omit<Armateur, 'id'>;
...
const ArmateurListPage: React.FC = () => {
    const { armateurs, deleteArmateur } = useArmateurs();
    
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingArmateurId, setEditingArmateurId] = useState<string | null>(null);
    ...
    const handleOpenModal = (armateurId: string | null = null) => {
        setEditingArmateurId(armateurId);
        setIsModalOpen(true);
    };

    const handleCloseModal = () => {
        setIsModalOpen(false);
        setEditingArmateurId(null);
    };

    const processedArmateurs = useMemo(() => {
        let filteredArmateurs = [...armateurs];

        if (searchTerm) {
            const lowercasedFilter = searchTerm.toLowerCase();
            filteredArmateurs = filteredArmateurs.filter(item =>
                item.armateur.toLowerCase().includes(lowercasedFilter) ||
        ...
        return filteredArmateurs;
    }, [armateurs, searchTerm, sortConfig]);
    
    const totalPages = Math.ceil(processedArmateurs.length / itemsPerPage);
    const paginatedArmateurs = useMemo(() => {
        const startIndex = (currentPage - 1) * itemsPerPage;
        return processedArmateurs.slice(startIndex, startIndex + itemsPerPage);
    }, [processedArmateurs, currentPage, itemsPerPage]);
    ...
    const handleDelete = (id: string, name: string) => {
        MySwal.fire({
            title: 'Êtes-vous sûr ?',
            text: `Vous êtes sur le point de supprimer l'armateur ${name}. Cette action est irréversible !`,
            icon: 'warning',
    ...
    const startEntry = processedArmateurs.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0;
    const endEntry = Math.min(currentPage * itemsPerPage, processedArmateurs.length);
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold text-slate-800">Gestion des Armateurs</h1>
                <button
            ...
            <div className="bg-white rounded-xl shadow-xl border border-slate-200">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-slate-600">
                        <thead className="text-xs font-semibold text-sidebar-text bg-sidebar-bg uppercase">
                            <tr>
                                <SortableHeader column="armateur" title="Armateur" sortConfig={sortConfig} requestSort={requestSort} />
                                <SortableHeader column="contact" title="Contact" sortConfig={sortConfig} requestSort={requestSort} />
                                <SortableHeader column="email" title="Email" sortConfig={sortConfig} requestSort={requestSort} />
                        ...
                        <tbody className="align-middle">
                            {paginatedArmateurs.length > 0 ? paginatedArmateurs.map((armateur, index) => (
                                <tr key={armateur.id} className={`border-t border-slate-200 hover:bg-primary/5 transition-colors duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/70'}`}>
                                    <th scope="row" className="px-6 py-3 font-medium text-slate-900 whitespace-nowrap">{armateur.armateur}</th>
                                    <td className="px-6 py-3">{armateur.contact}</td>
                                    <td className="px-6 py-3">{armateur.email}</td>
                                    <td className="px-6 py-3">
                                        <div className="flex items-center justify-center space-x-2">
                                            <button onClick={() => handleOpenModal(armateur.id)} className="p-1.5 text-blue-600 bg-blue-100 hover:bg-blue-200 rounded-md transition-colors" aria-label={`Modifier l'armateur ${armateur.armateur}`}>
                                                <EditIconAlt className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => handleDelete(armateur.id, armateur.armateur)} className="p-1.5 text-red-600 bg-red-100 hover:bg-red-200 rounded-md transition-colors" aria-label={`Supprimer l'armateur ${armateur.armateur}`}>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center p-4 border-t border-slate-200 rounded-b-xl">
                    <p className="text-sm text-slate-600 mb-4 sm:mb-0">
                        Affichage de <span className="font-semibold">{startEntry}</span> à <span className="font-semibold">{endEntry}</span> sur <span className="font-semibold">{processedArmateurs.length}</span> éléments
                    </p>
                    {totalPages > 1 && (
                         <nav>
            ...
            <ArmateurFormModal isOpen={isModalOpen} onClose={handleCloseModal} armateurId={editingArmateurId} />
        </div>
    );
};
